...
export default function BarChartForm({ chart, chartConfig, setForm }) {
    ...
    useEffect(() => {
        ...
        setFormBar({
            ...
            max: chartConfig?.max,
            data: [
                { bardata: chartConfig.data[0]?.bardata, color: chartConfig.data[0]?.color }
    ...
    return (
        ...
        <>
            ...
            <div className='flex items-start flex-col gap-1 w-full'>
                ...
                <input onChange={handleBarForm} className='w-20 pl-2' name='min' placeholder={chartConfig?.min ? chartConfig?.min : 0} type="number" />
            </div>
            <div className='flex items-start flex-col gap-1 w-full' >
                <label className='text-white' htmlFor="max">Max Y</label>
                <input onChange={handleBarForm} placeholder={chartConfig?.max ? chartConfig?.max : null} className='w-20' name='max' type="number" />
            </div>
                    ...
                    <div className='flex lef witems-start flex-col ' >
                        ...
                        <select required onChange={handleBarForm} name="bardata" id="">  <option value="">-</option>{
                            Object.keys(chart?.data[0]).map(key => {
                                ...
                                return <option selected={chartConfig?.data[0].bardata === key ? true : false} value={key}>{key}</option>
                            })
                        }</select>
